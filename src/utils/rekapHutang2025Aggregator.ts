...
export interface RekapPosBelanjaItem {
  id: string;
  noUrut: number;
  kodeRekening: string;
  kegiatan: string; // Uraian Pos Belanja
  namaPerusahaan: string;
  tahun: string;
  jenisSumber: 'BLUD' | 'APBD';
  noPoSpk: string;
  tanggalInvoice: string;
  totalTagihan: number; // Saldo Awal Hutang 2025
  koreksi: number;
  jumlahBayar: number; // Realisasi Pembayaran
  sisaHutang: number; // Saldo Akhir
...
export const MASTER_31_POS_BELANJA: {
  noUrut: number;
  kodeRekening: string;
  uraian: string;
  isHighlighted?: boolean;
  ...
  { noUrut: 80, kodeRekening: '5.1.01.01.01.0001', uraian: 'Belanja Gaji Pegawai BLUD - Desember 2025' },
  { noUrut: 81, kodeRekening: '5.1.01.01.01.0001', uraian: 'Jasa Pelayanan Medis Dokter' }
];

export function matchInvoiceToPosBelanja(inv: InvoiceHutang2025Record): {
  kodeRekening: string;
  uraian: string;
  noUrut?: number;
  isHighlighted?: boolean;
  ...
  // 1. Direct match against 31 master categories
  for (const master of MASTER_31_POS_BELANJA) {
    if (master.uraian.toLowerCase() === sub.toLowerCase() || master.uraian.toLowerCase() === uraian.toLowerCase()) {
      return master;
    }
  }

  // 2. Keyword & Alias matches
  const combined = `${sub} ${uraian}`.toLowerCase();

  if (combined.includes('bmhp') || combined.includes('farmasi')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Farmasi'));
    if (found) return found;
  }

  if (combined.includes('obat') || combined.includes('vaksin') || combined.includes('antibiotik')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian === 'Belanja Obat-Obatan-Obat');
    if (found) return found;
  }

  if (combined.includes('aplikasi') || combined.includes('simrs') || combined.includes('sim rs') || combined.includes('konversi')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Konversi Aplikasi'));
    if (found) return found;
  }

  if (combined.includes('sparepart') || combined.includes('elektromedik')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Sparepart Perbaikan Alat Kesehatan'));
    if (found) return found;
  }

  if (combined.includes('(apd)') || combined.includes('handscoon') || combined.includes('masker bedah') || combined.includes('infus set')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('(APD)'));
    if (found) return found;
  }

  if (combined.includes('pemeliharaan gedung') || combined.includes('bangunan gedung')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Pemeliharaan Bangunan Gedung'));
    if (found) return found;
  }

  if (combined.includes('konsultansi') || combined.includes('arsitektur')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Konsultansi Pengawasan Arsitektur'));
    if (found) return found;
  }

  if (combined.includes('modal alat studio') || combined.includes('alat studio')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Modal Alat Studio'));
    if (found) return found;
  }

  if (combined.includes('modal alat rumah tangga') || combined.includes('home use')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Modal Alat Rumah Tangga'));
    if (found) return found;
  }

  if (combined.includes('modal alat kesehatan') || combined.includes('alat kesehatan umum')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Modal Alat Kesehatan Umum'));
    if (found) return found;
  }

  if (combined.includes('kebersihan') || combined.includes('laundry') || combined.includes('pembersih') || combined.includes('desinfektan')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Peralatan Kebersihan'));
    if (found) return found;
  }

  if (combined.includes('dialisis') || combined.includes('hemodialisa')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Dialisis'));
    if (found) return found;
  }

  if (combined.includes('laboratorium') || combined.includes('reagen')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Alat Laboratorium'));
    if (found) return found;
  }

  if (combined.includes('bahan cetak') || combined.includes('cetakan') || combined.includes('rekam medis')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Bahan Cetak'));
    if (found) return found;
  }

  if (combined.includes('alat tulis kantor') || combined.includes('atk') || combined.includes('thermal')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Alat Tulis Kantor'));
    if (found) return found;
  }

  if (combined.includes('jaringan listrik') || combined.includes('genset') || combined.includes('kelistrikan')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Jaringan-Jaringan Listrik'));
    if (found) return found;
  }

  if (combined.includes('gaji pegawai')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Gaji Pegawai'));
    if (found) return found;
  }

  if (combined.includes('pelayanan medis') || combined.includes('dokter')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Pelayanan Medis Dokter'));
    if (found) return found;
  }

  if (combined.includes('kawat') || combined.includes('faksimili') || combined.includes('internet') || combined.includes('langganan')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Kawat/Faksimili'));
    if (found) return found;
  }

  if (combined.includes('tabung gas')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Tabung Gas'));
    if (found) return found;
  }

  if (combined.includes('tenaga kebersihan')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Tenaga Kebersihan'));
    if (found) return found;
  }

  if (combined.includes('tenaga keamanan')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Tenaga Keamanan'));
    if (found) return found;
  }

  if (combined.includes('tagihan air')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Tagihan Air'));
    if (found) return found;
  }

  if (combined.includes('tagihan listrik')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Tagihan Listrik'));
    if (found) return found;
  }

  if (combined.includes('pajak')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Pajak'));
    if (found) return found;
  }

  if (combined.includes('air limbah')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Air Limbah'));
    if (found) return found;
  }

  if (combined.includes('alat angkutan')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Alat Angkutan'));
    if (found) return found;
  }

  if (combined.includes('alat bengkel')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Alat Bengkel'));
    if (found) return found;
  }

  if (combined.includes('pembangkit listrik')) {
    const found = MASTER_31_POS_BELANJA.find(p => p.uraian.includes('Pembangkit Listrik'));
    if (found) return found;
  }

  // 3. Fallback to INITIAL_KODE_REKENING
  for (const kd of INITIAL_KODE_REKENING) {
    if (kd.uraian.toLowerCase() === sub.toLowerCase() || kd.uraian.toLowerCase() === uraian.toLowerCase()) {
      return {
  ...
  // 4. Default fallback
  const label = sub || uraian || 'Belanja Lainnya';
  return {
    kodeRekening: '-',
    uraian: label
  };
...
export function aggregateRekapHutang2025(invoices: InvoiceHutang2025Record[]): RekapPosBelanjaItem[] {
  // Initialize map with all 31 master pos belanja
  const posMap = new Map<string, RekapPosBelanjaItem>();

  MASTER_31_POS_BELANJA.forEach(master => {
    posMap.set(master.uraian, {
      id: `rekap-2025-master-${master.noUrut}`,
      noUrut: master.noUrut,
      kodeRekening: master.kodeRekening,
      kegiatan: master.uraian,
      namaPerusahaan: 'REKANAN BLUD',
      tahun: '2025',
      jenisSumber: 'BLUD',
      noPoSpk: '',
      tanggalInvoice: '2025-12-31',
      totalTagihan: 0,
      koreksi: 0,
      jumlahBayar: 0,
      sisaHutang: 0,
  ...
  invoices.forEach(inv => {
    const matched = matchInvoiceToPosBelanja(inv);
    const key = matched.uraian;

    if (!posMap.has(key)) {
      posMap.set(key, {
        id: `rekap-2025-extra-${posMap.size + 1}`,
        noUrut: matched.noUrut || (posMap.size + 1),
        kodeRekening: matched.kodeRekening || '-',
        kegiatan: matched.uraian,
        namaPerusahaan: 'REKANAN BLUD',
        tahun: '2025',
        jenisSumber: (inv.sumberAnggaran === 'APBD' ? 'APBD' : 'BLUD'),
        noPoSpk: '',
        tanggalInvoice: '2025-12-31',
        totalTagihan: 0,
        koreksi: 0,
    ...
    const invJumlah = inv.jumlahInvoice || 0;
    const invKoreksi = inv.koreksi || 0;
    const invBayar = inv.pembayaran || 0;
    const invSisa = inv.sisaHutang !== undefined ? inv.sisaHutang : Math.max(0, invJumlah + invKoreksi - invBayar);
  ...
  // Convert to array and format statuses
  const results = Array.from(posMap.values()).map(item => {
    const finalSisa = item.sisaHutang;
    return {
      ...item,
      status: (finalSisa <= 0 ? 'Lunas' : 'Belum Lunas') as 'Lunas' | 'Belum Lunas'
    };
  });
