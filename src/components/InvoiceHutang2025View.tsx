...
import { MASTER_31_POS_BELANJA, matchInvoiceToPosBelanja } from '../utils/rekapHutang2025Aggregator';
...
// Helper to resolve the correct Kode Rekening for a 2025 invoice record
export const getEffectiveKodeRekening = (item: InvoiceHutang2025Record): string => {
  if (item.kodeRekening && item.kodeRekening.trim() && item.kodeRekening.trim() !== '-') {
    return item.kodeRekening.trim();
  }
  const matched = matchInvoiceToPosBelanja(item);
...
export const InvoiceHutang2025View: React.FC<InvoiceHutang2025ViewProps> = ({
  ...
  const [filterRekanan, setFilterRekanan] = useState<string>(() => {
    try {
      const saved = sessionStorage.getItem('rsud_filter_rekanan_2025');
      if (saved) {
        sessionStorage.removeItem('rsud_filter_rekanan_2025');
        return saved;
      }
    } catch (e) {}
    return 'ALL';
  ...
  const uniqueBulan = useMemo(() => {
    ...
    data.forEach(d => {
      ...
      if (bInv && bInv.trim()) {
        set.add(bInv.trim().toUpperCase());
      }
      const bRek = getEffectiveBulanRekap(d);
      if (bRek && bRek.trim()) {
        set.add(bRek.trim().toUpperCase());
      }
    });
  ...
  const filteredItems = useMemo(() => {
    return data.filter(item => {
      ...
      // Bulan filter (check effective bulanInvoice or effective bulanRekap)
      if (filterBulan !== 'ALL') {
      ...
      // Month sorting (Bulan Invoice, Bulan Rekap, or Bulan SPD)
      if (sortField === 'bulanInvoice' || sortField === ('bulanRekap' as any) || sortField === 'bulanSpd') {
        let valMonthA = '';
        let valMonthB = '';
        if (sortField === 'bulanInvoice') {
          valMonthA = getEffectiveBulanInvoice(a);
          valMonthB = getEffectiveBulanInvoice(b);
        } else if (sortField === ('bulanRekap' as any)) {
          valMonthA = getEffectiveBulanRekap(a);
          valMonthB = getEffectiveBulanRekap(b);
        } else {
          valMonthA = (a.bulanSpd || '') as string;
          valMonthB = (b.bulanSpd || '') as string;
        }
      ...
      // Sort by Umur Hutang dynamically
      ...
      if (['tglInvoice', 'tglRekap', 'tglTandaTerima', 'tglMasukSpj', 'tglSpbSpk', 'jatuhTempo', 'tglBayar', 'tglSpdBukuKas'].includes(String(sortField))) {
        const valA = sortField === 'jatuhTempo' ? (getEffectiveJatuhTempo(a) || a.jatuhTempo) : (a[sortField as keyof InvoiceHutang2025Record] as string);
        const valB = sortField === 'jatuhTempo' ? (getEffectiveJatuhTempo(b) || b.jatuhTempo) : (b[sortField as keyof InvoiceHutang2025Record] as string);
  ...
  const handleOpenEdit = (record: InvoiceHutang2025Record) => {
    ...
    const effBulanRekap = getMonthNameFromDate(effRekap) || record.bulanRekap || '';
    const effInvoice = record.tglInvoice || '';
    const effBulanInvoice = getMonthNameFromDate(effInvoice) || record.bulanInvoice || '';
    const effJatuhTempo = (record.jatuhTempo && record.jatuhTempo !== '-' && record.jatuhTempo !== '0')
      ? record.jatuhTempo
      : (getEffectiveJatuhTempo(record) || (effInvoice ? addOneMonthToDateStr(effInvoice) : ''));
    const effUmur = calculateUmurHutang({
      ...record,
      jatuhTempo: effJatuhTempo,
  ...
  const handleSaveForm = (e: React.FormEvent) => {
    ...
    const sanitizedData = nextData.map(sanitizeInvoiceRecord);
    inMemoryInvoice2025Cache = sanitizedData;
    setData(sanitizedData);
    setIsFormOpen(false);

    // Save immediately and synchronously across storage layers
    idbSet(IDB_KEY_INVOICE_2025, sanitizedData).catch(err => {
      console.warn('[IDB] Error persisting invoice 2025:', err);
    });
    window.dispatchEvent(new CustomEvent('rsud_invoice_hutang_2025_updated', { detail: sanitizedData }));
  };
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredItems.map(item => {
      ...
      const jtDisplay = (item.jatuhTempo && item.jatuhTempo !== '-' && item.jatuhTempo !== '0')
        ? item.jatuhTempo
        : (getEffectiveJatuhTempo(item) || '');
  ...
  const handleImportSuccess = (importedRecords: InvoiceHutang2025Record[], mode: 'replace' | 'append') => {
    ...
    const sanitizedFinalData = finalData.map(sanitizeInvoiceRecord);
    inMemoryInvoice2025Cache = sanitizedFinalData;
    setData(sanitizedFinalData);
    idbSet(IDB_KEY_INVOICE_2025, sanitizedFinalData).catch(err => console.warn('[IDB] error on import:', err));
    window.dispatchEvent(new CustomEvent('rsud_invoice_hutang_2025_updated', { detail: sanitizedFinalData }));
    if (onShowToast) {
      onShowToast(
        mode === 'replace'
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-teal-900 via-emerald-900 to-slate-950 text-white rounded-2xl p-6 shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border border-teal-700/50">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Template Excel */}
          <button
            onClick={() => downloadInvoiceHutangExcelTemplate(2025)}
          ...
          <table className="w-full text-xs text-left border-collapse whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-zinc-800/80">
              {paginatedItems.length === 0 ? (
                <tr>
                  <td colSpan={22} className="px-6 py-12 text-center text-slate-500 dark:text-zinc-400">
                ...
                paginatedItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-slate-600 dark:text-zinc-400 font-mono text-[11px]">
                        {item.jatuhTempo && item.jatuhTempo !== '-' ? item.jatuhTempo : (getEffectiveJatuhTempo(item) || '-')}
                      </td>
                      ...
                      <td className="px-3 py-2.5 text-center font-mono font-medium">
                        {(() => {
                          const u = calculateUmurHutang(item);
                          if (item.sisaHutang <= 0) {
                            return <span className="text-slate-400 dark:text-zinc-500 font-sans text-[11px]">Lunas</span>;
                          }
                          if (u <= 0) {
                            return <span className="px-2 py-0.5 rounded-full text-[10.5px] font-semibold bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">Belum JT</span>;
                          }
                          if (u <= 30) {
                            return <span className="px-2 py-0.5 rounded-full text-[10.5px] font-semibold bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300 border border-blue-200 dark:border-blue-800">{u} hr</span>;
                          }
                          if (u <= 60) {
                            return <span className="px-2 py-0.5 rounded-full text-[10.5px] font-semibold bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300 border border-amber-200 dark:border-amber-800">{u} hr</span>;
                          }
                          if (u <= 90) {
                            return <span className="px-2 py-0.5 rounded-full text-[10.5px] font-semibold bg-orange-50 text-orange-700 dark:bg-orange-950/40 dark:text-orange-300 border border-orange-200 dark:border-orange-800">{u} hr</span>;
                          }
                          return <span className="px-2 py-0.5 rounded-full text-[10.5px] font-bold bg-rose-100 text-rose-700 dark:bg-rose-950/60 dark:text-rose-300 border border-rose-300 dark:border-rose-800 animate-pulse">{u} hr</span>;
                        })()}
                      </td>
                  ...
                  <div className="bg-slate-50 dark:bg-zinc-900/60 p-3 rounded-xl border border-slate-200 dark:border-zinc-800 space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const effectiveJT = formValues.jatuhTempo || (iso ? addOneMonthToDateStr(iso) : '');
                        const calculatedUmur = calculateUmurHutang({
                          jatuhTempo: effectiveJT,
                          tglInvoice: display,
                          sisaHutang: formValues.sisaHutang ?? 1
                        });
                        setFormValues({
                          ...formValues,
                          tglInvoice: display,
                  ...
                  <div className="bg-slate-50 dark:bg-zinc-900/60 p-3 rounded-xl border border-slate-200 dark:border-zinc-800 space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const iso = e.target.value;
                        const display = fromInputDate(iso);
                        const effectiveJT = display || (formValues.tglInvoice ? addOneMonthToDateStr(formValues.tglInvoice) : '');
                        const calculatedUmur = calculateUmurHutang({
                          jatuhTempo: effectiveJT,
                    ...
                    />
                    <div className="text-[11px] text-slate-500 dark:text-zinc-400 flex items-center justify-between">
                      <span>Umur: <strong className="text-rose-600 dark:text-rose-400">{formValues.lamaHariHutang || 0} hari</strong></span>
                      <span className="font-mono">{formValues.jatuhTempo || (formValues.tglInvoice ? `Auto: ${addOneMonthToDateStr(formValues.tglInvoice)}` : '-')}</span>
                    </div>
                  </div>
