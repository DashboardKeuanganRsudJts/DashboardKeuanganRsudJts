...
interface InvoiceHutang2026ViewProps {
  user?: User | null;
  role?: string;
  isAdmin?: boolean;
...
let inMemoryInvoice2026Cache: InvoiceHutang2026Record[] | null = null;

// Helper to ensure mathematical integrity, General format for invoice number, and preserve user edits
const sanitizeInvoiceRecord = (item: InvoiceHutang2026Record): InvoiceHutang2026Record => {
...
export const InvoiceHutang2026View: React.FC<InvoiceHutang2026ViewProps> = ({
  user,
  role,
  isAdmin,
  onShowToast
  ...
  const [data, setData] = useState<InvoiceHutang2026Record[]>(() => {
    if (inMemoryInvoice2026Cache && Array.isArray(inMemoryInvoice2026Cache) && inMemoryInvoice2026Cache.length > 0) {
      return inMemoryInvoice2026Cache;
    }
    try {
      if (typeof window !== 'undefined' && window.localStorage) {
        const saved = localStorage.getItem(IDB_KEY_INVOICE_2026);
        if (saved) {
          const parsed = JSON.parse(saved);
          if (Array.isArray(parsed) && parsed.length > 0) {
            const cleaned = parsed.map(sanitizeInvoiceRecord);
            inMemoryInvoice2026Cache = cleaned;
            return cleaned;
          }
        }
      }
    } catch (e) {
      console.warn('[Invoice2026] Initial localStorage read error:', e);
    }
    const init = INITIAL_INVOICE_HUTANG_2026.map(sanitizeInvoiceRecord);
    inMemoryInvoice2026Cache = init;
    return init;
  });
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    idbGet<InvoiceHutang2026Record[]>(IDB_KEY_INVOICE_2026)
      .then(saved => {
        if (isMounted) {
      ...
      .catch(err => {
        console.warn('[IDB] Error reading invoice 2026:', err);
  ...
  useEffect(() => {
    if (!isLoaded) return;
    inMemoryInvoice2026Cache = data;
    idbSet(IDB_KEY_INVOICE_2026, data).catch(err => {
      console.warn('[IDB] Error writing invoice 2026:', err);
    });
    window.dispatchEvent(new CustomEvent('rsud_invoice_hutang_2026_updated', { detail: data }));
  }, [data, isLoaded]);
  ...
  // Sorting
  const [sortField, setSortField] = useState<keyof InvoiceHutang2026Record>('no');
  const [sortAsc, setSortAsc] = useState(true);

  // Modal states
  const [selectedRecord, setSelectedRecord] = useState<InvoiceHutang2026Record | null>(null);
  ...
  const handleSort = (field: keyof InvoiceHutang2026Record) => {
    if (sortField === field) {
      setSortAsc(!sortAsc);
    } else {
      setSortField(field);
  ...
  const handleOpenDetail = (record: InvoiceHutang2026Record) => {
  ...
  const handleDeleteRecord = (id: string, no: number) => {
    if (window.confirm(`Apakah Anda yakin ingin menghapus data Invoice No. ${no}?`)) {
      ...
      setData(nextData);
      idbSet(IDB_KEY_INVOICE_2026, nextData).catch(err => console.warn('[IDB] error on delete:', err));
  ...
  const handleToggleLunasInvoice = (item: InvoiceHutang2026Record) => {
    ...
    inMemoryInvoice2026Cache = nextData;
    setData(nextData);
    idbSet(IDB_KEY_INVOICE_2026, nextData).catch(err => console.warn('[IDB] error on toggle:', err));
    window.dispatchEvent(new CustomEvent('rsud_invoice_hutang_2026_updated', { detail: nextData }));
    if (onShowToast) {
  ...
  const handleSaveForm = (e: React.FormEvent) => {
    ...
    let nextData: InvoiceHutang2026Record[];
    ...
    if (isEditMode && formValues.id) {
      ...
      nextData = data.map(item => {
        if (item.id === formValues.id) {
          return {
            ...item,
            ...(formValues as InvoiceHutang2026Record),
            bagian: finalBidang,
            bidang: finalBidang,
      ...
      // Insert new record
      const newId = `INV-2026-${(formValues.no || data.length + 1).toString().padStart(4, '0')}`;
      const newRecord: InvoiceHutang2026Record = {
        id: newId,
  ...
  const handleRefreshNominal = () => {
    ...
    if (onShowToast) {
      onShowToast('Nominal seluruh Invoice 2026 berhasil disinkronkan & diperbarui!', 'success');
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `INVOICE_HUTANG_2026_RSUD_JATISARI_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Invoice Hutang 2026');
    XLSX.writeFile(wb, `INVOICE_HUTANG_2026_RSUD_JATISARI_${new Date().toISOString().slice(0, 10)}.xlsx`);

    if (onShowToast) {
      onShowToast(`Berhasil mengekspor ${filteredItems.length} data invoice 2026 ke file Excel (.xlsx)`, 'success');
    }
  };

  const handleImportSuccess = (importedRecords: InvoiceHutang2026Record[], mode: 'replace' | 'append') => {
    let finalData: InvoiceHutang2026Record[] = [];
    if (mode === 'replace') {
    ...
    setData(finalData);
    idbSet(IDB_KEY_INVOICE_2026, finalData).catch(err => console.warn('[IDB] error on import:', err));
    window.dispatchEvent(new CustomEvent('rsud_invoice_hutang_2026_updated', { detail: finalData }));
    if (onShowToast) {
      onShowToast(
        mode === 'replace'
          ? `Berhasil mengimpor ${importedRecords.length} data invoice 2026 dari Excel (Mode Ganti).`
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-teal-900 via-emerald-900 to-slate-950 text-white rounded-2xl p-6 shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border border-teal-700/50">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal-500/20 text-teal-300 text-xs font-semibold mb-2 border border-teal-400/30">
            <FileSpreadsheet className="w-3.5 h-3.5 text-teal-300" /> DATA MASTER INVOICE 2026
          </div>
          <h2 className="text-2xl font-black tracking-tight text-white flex items-center gap-2">
            INVOICE HUTANG TAHUN 2026
          </h2>
          <p className="text-teal-100/80 text-xs mt-1 max-w-2xl leading-relaxed">
            Data detail buku register faktur/invoice pengadaan barang, obat-obatan, BMHP, dan jasa Rumah Sakit Umum Daerah Jatisari TA 2026. Total tercatat {data.length} transaksi invoice.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-[#0d1216] rounded-2xl p-4 border border-slate-200 dark:border-emerald-950/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-[#12181f] border border-slate-200 dark:border-zinc-800 rounded-xl text-xs font-medium text-slate-700 dark:text-zinc-300">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setSortField(field as keyof InvoiceHutang2026Record);
                setSortAsc(dir === 'asc');
                setCurrentPage(1);
              }}
          ...
          {/* Page size */}
          <select
            id="invoice-2026-page-size-select"
            value={pageSize}
            onChange={(e) => {
      ...
      <div className="bg-white dark:bg-[#0d1216] rounded-2xl border border-slate-200 dark:border-emerald-950/80 shadow-sm overflow-hidden p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="text-xs font-bold text-slate-700 dark:text-zinc-300">
            Daftar Lengkap Invoice Hutang 2026 ({filteredItems.length} Data)
          </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-base">
                    {isEditMode ? `Edit Invoice No. ${formValues.no} (TA 2026)` : 'Tambah Data Invoice 2026 Baru'}
                  </h3>
            ...
            <form onSubmit={handleSaveForm} className="p-6 overflow-y-auto space-y-6 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                  ...
                  <div className="sm:col-span-9">
                    ...
                    <input
                      ...
                      required
                    />
                    <datalist id="rekanan-options-2026">
                  ...
                  <div className="sm:col-span-6">
                    ...
                    <input
                      ...
                      placeholder="Contoh: INV/2026/001 atau T.HCA.0180.26.MDP"
                      required
                    />
                  </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="bg-slate-50 dark:bg-zinc-900/60 p-3 rounded-xl border border-slate-200 dark:border-zinc-800 space-y-2">
                    ...
                    <select
                      value={formValues.bulanInvoice || 'JANUARI'}
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 pb-1.5 border-b border-slate-200 dark:border-zinc-800">
                  ...
                  <h4 className="font-bold text-slate-900 dark:text-white text-xs uppercase tracking-wide">
                    4. Nilai Finansial, Realisasi & Buku Kas (TA 2026)
                  </h4>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-slate-50 dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 rounded-xl font-mono"
                      placeholder="SPD-LS/RSUD Jatisari/I/2026/00028"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 pt-2 bg-slate-50 dark:bg-zinc-900/60 p-3 rounded-xl border border-slate-200 dark:border-zinc-800">
                  ...
                  />
                  <label htmlFor="masukBukuKas2026" className="font-semibold text-slate-800 dark:text-zinc-200 cursor-pointer select-none">
                    Sudah Masuk Buku Kas & Dicatat dalam Realisasi Pembukuan TA 2026
                  </label>
                </div>
              </div>
              ...
              <div className="px-6 py-4 bg-slate-100 dark:bg-zinc-900 border-t border-slate-200 dark:border-zinc-800 flex justify-end gap-2 -mx-6 -mb-6 mt-6">
                ...
                >
                  <Save className="w-4 h-4" /> Simpan Data Invoice 2026
                </button>
              </div>
            </form>
          </div>
